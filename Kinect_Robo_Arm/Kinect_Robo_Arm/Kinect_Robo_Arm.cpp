...
// Definitions for various body parts and for making the code easier to read
#define PI 3.14159
#define body myFrame.SkeletonData[a]
...
int main()
{
    ...
    while (1) {
        ...
        for (int a = 0; a < 6; a++) {
            if (myFrame.SkeletonData[a].eTrackingState == NUI_SKELETON_TRACKED) {
                // may or may not help
                ...
                /***** Base *****/
                angle = getAngle(LSHOULDER.x - RSHOULDER.x, LSHOULDER.y - RSHOULDER.y, LSHOULDER.z - RSHOULDER.z, ELBOW.x - RSHOULDER.x, ELBOW.y - RSHOULDER.y, ELBOW.z - RSHOULDER.z);
                ...
                if (temp != temp){
                    ...
                    cout << "Base angle: " << (int)serialBuffer[0] << endl;
				}

                /***** Shoulder *****/
                angle = getAngle(HIP.x - CSHOULDER.x, HIP.y - CSHOULDER.y, HIP.z - CSHOULDER.z,ELBOW.x - RSHOULDER.x, ELBOW.y - RSHOULDER.y, ELBOW.z - RSHOULDER.z);
                ...
                if (temp != temp){
					cout << "Shoulder: bad angle\n";
				}else{
                    cout << "Shoulder angle: " << (int)serialBuffer[1] << endl;
				}

                /***** Elbow *****/
                angle = getAngle(RSHOULDER.x - ELBOW.x, RSHOULDER.y - ELBOW.y, RSHOULDER.z - ELBOW.z,WRIST.x - ELBOW.x, WRIST.y - ELBOW.y, WRIST.z - ELBOW.z);
                ...
                if (temp != temp){
                    cout << "Elbow: bad angle\n";
				}else{
                    cout << "Elbow angle: " << (int)serialBuffer[2] << endl;
				}
                ...
                cout << "Right Hand: ";
                cout << myFrame.SkeletonData[a].SkeletonPositions[NUI_SKELETON_POSITION_HAND_RIGHT].y << endl;
